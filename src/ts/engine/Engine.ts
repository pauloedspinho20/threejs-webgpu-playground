...
export class Engine implements EngineContext {
  ...
  constructor(options: EngineOptions = {}) {
    ...
    this.renderer = new THREE.WebGPURenderer({
      antialias: opts.renderer.antialias,
    });
    ...
    this.physicsWorld.gravity.set(
      opts.physics.gravity[0],
      opts.physics.gravity[1],
      opts.physics.gravity[2],
    );
    this.physicsWorld.broadphase = new CANNON.SAPBroadphase(this.physicsWorld);
    (this.physicsWorld.solver as CANNON.GSSolver).iterations =
      opts.physics.solverIterations;
    ...
    if (opts.world !== null) {
      ...
      loadingManager.onFinishedCallback = () => {
        this.update(1, 1);
        this.setTimeScale(1);
        this.events.emit("world:loaded", {
          scenarios: this.getScenarioInfos(),
        });
      };
  ...
  public resize(): void {
    ...
    const height =
      container === document.body ? window.innerHeight : container.clientHeight;
  ...
  public on<K extends keyof EngineEvents>(
    type: K,
    cb: (payload: EngineEvents[K]) => void,
  ): () => void {
  ...
  public loadScene(loadingManager: LoadingManager, gltf: GLTF): void {
    ...
    if (defaultScenario !== undefined)
      this.launchScenario(defaultScenario.id, loadingManager);
  }
    ...
    this.sceneLoader.onUserData("data", "physics", ({ node, data, ctx }) => {
      ...
      if (data.type === "box") {
        ...
        const phys = new BoxCollider({
          size: new THREE.Vector3(worldScale.x, worldScale.y, worldScale.z),
        });
        phys.body.position.copy(
          Utils.cannonVector(node.getWorldPosition(new THREE.Vector3())),
        );
        phys.body.quaternion.copy(
          Utils.cannonQuat(node.getWorldQuaternion(new THREE.Quaternion())),
        );
  ...
  public launchScenario(
    ...
    if (
      launched !== undefined &&
      !launched.spawnAlways &&
      launched.initialCameraAngle !== undefined
    ) {
    ...
    if (loadingManager.onFinishedCallback === undefined) {
      const welcome =
        launched !== undefined &&
        !launched.spawnAlways &&
        launched.descriptionTitle !== undefined
          ? {
              title: launched.descriptionTitle,
              content: launched.descriptionContent,
            }
          : undefined;
      loadingManager.onFinishedCallback = () => {
        this.setTimeScale(1);
